...
<div class="content">
    <div style="background:#fff;border-radius:15px;padding:25px;box-shadow:0 4px 15px rgba(0,0,0,.08);max-width:600px">
        ...
        <form method="POST" action="{{ route('admin.produk.update', $produk->id_produk) }}">@csrf @method('PUT')
            ...
            <div class="d-flex gap-2"><button type="submit" class="btn btn-primary flex-fill shadow-sm fw-bold" style="border-radius:10px">Simpan</button><a href="{{ route('admin.produk') }}" class="btn btn-outline-secondary shadow-sm" style="border-radius:10px">Batal</a></div>
        </form>
    </div>
</div>
@endsection
