...
<html lang="id">
<head>
<title>@yield('title', 'Admin Panel') - Tirta Kencana</title>
...
.bg-orange-light{background:#fff3e0;color:#ef6c00}.bg-green-light{background:#e8f5e9;color:#2e7d32}
...
.table-header{background:linear-gradient(90deg,#1976d2,#42a5f5);color:#fff;padding:15px;font-weight:600}
...
<div class="sidebar-desktop d-none d-md-block">
    <div class="d-flex align-items-center gap-2 mb-3">
        <img src="{{ asset('logo.png') }}" class="logo" onerror="this.style.display='none'">
        <div><b>Tirta Kencana</b><br><small>Admin Panel</small></div>
    </div>
    <hr>
    <a href="{{ route('admin.dashboard') }}" class="{{ str_contains($currentRoute, 'admin.dashboard') ? 'active' : '' }}"><i class="bi bi-speedometer2"></i> Dashboard</a>
    <a href="{{ route('admin.user') }}" class="{{ str_contains($currentRoute, 'admin.user') ? 'active' : '' }}"><i class="bi bi-people"></i> Data User</a>
    <a href="{{ route('admin.transaksi') }}" class="{{ str_contains($currentRoute, 'admin.transaksi') ? 'active' : '' }}"><i class="bi bi-receipt"></i> Data Transaksi</a>
    <a href="{{ route('admin.produk') }}" class="{{ str_contains($currentRoute, 'admin.produk') ? 'active' : '' }}"><i class="bi bi-box-seam"></i> Data Produk</a>
    <a href="{{ route('admin.pengaturan') }}" class="{{ str_contains($currentRoute, 'admin.pengaturan') ? 'active' : '' }}"><i class="bi bi-gear"></i> Pengaturan</a>
    <hr>
...
<div class="topbar d-flex justify-content-between align-items-center">
    <div class="d-flex align-items-center gap-2">
        ...
        <div style="line-height:1.1"><b>Administrator</b><br><small>{{ session('nama') }}</small></div>
    </div>
...
<div class="offcanvas offcanvas-start" id="menuMobile" tabindex="-1">
    <div class="sidebar-mobile">
        <div class="d-flex justify-content-between align-items-center mb-4 text-white">
            <div class="d-flex align-items-center gap-2"><img src="{{ asset('logo.png') }}" class="logo" onerror="this.style.display='none'"><div><b>Tirta Kencana</b><br><small>Admin Panel</small></div></div>
