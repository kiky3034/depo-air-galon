...
<div class="content">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <a href="{{ route('pegawai.dashboard') }}" class="btn btn-outline-primary btn-sm shadow-sm px-3" style="border-radius:10px"><i class="bi bi-arrow-left me-1"></i>Dashboard</a>
        <a href="{{ route('pegawai.produk.tambah') }}" class="btn btn-primary btn-sm shadow-sm px-3" style="border-radius:10px"><i class="bi bi-plus-lg me-1"></i>Tambah Produk</a>
    </div>
            ...
            <table class="table table-hover align-middle text-center" style="font-size:13px">
                ...
                <tr>
                    ...
                    <td><div class="d-flex gap-1 justify-content-center">
                        <a href="{{ route('pegawai.produk.edit', $row->id_produk) }}" class="btn btn-warning btn-sm text-white shadow-sm" style="border-radius:8px"><i class="bi bi-pencil-square"></i></a>
                        <form method="POST" action="{{ route('pegawai.produk.delete', $row->id_produk) }}" onsubmit="return confirm('Yakin hapus?')">@csrf @method('DELETE')<button class="btn btn-danger btn-sm shadow-sm" style="border-radius:8px"><i class="bi bi-trash"></i></button></form>
                    </div></td>
                </tr>
                @endforeach
                </tbody>
